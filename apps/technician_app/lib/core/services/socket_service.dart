...
class TechnicianSocketService {
  ...
  void _attemptConnect() {
    ...
    try {
      ...
      _socket!.on('TECHNICIAN_BOOKING_REQUEST', (data) {
        debugPrint('🚨 [TechnicianSocket] Incoming Booking Request: $data');
        _handleIncomingJobAlert(data is Map ? Map<String, dynamic>.from(data) : {});
      });
      ...
      _socket!.on('booking:cancelled', (data) {
        final bookingId = data is Map ? (data['bookingId'] ?? data['id'])?.toString() ?? '' : '';
        BookingRequestManager().handleProposalExpiredOrClaimed(bookingId);
      });

      _socket!.on('booking:claimed', (data) {
        final bookingId = data is Map ? (data['bookingId'] ?? data['id'])?.toString() ?? '' : '';
        BookingRequestManager().handleProposalExpiredOrClaimed(bookingId);
      });
      ...
      _socket!.on('dispatch:expired', (data) {
        final bookingId = data is Map ? (data['bookingId'] ?? data['id'])?.toString() ?? '' : '';
        BookingRequestManager().handleProposalExpiredOrClaimed(bookingId);
      });
  ...
  void _joinRooms() {
    ...
    final cat = _currentCategory.toLowerCase().replaceFirst('category_', '').replaceFirst('cat_', '');
  ...
  /// Triggers centralized BookingRequestManager for audible alert and in-app popup
  void _handleIncomingJobAlert(Map<String, dynamic> data) {
    if (data.isEmpty) return;

    // Show system notification banner if background
    NotificationService().showJobAlertNotification(data);

    // Route to BookingRequestManager for in-app popup across any screen
