            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Real-time Service-Specific 15 KM Availability Badge
                  Builder(
                    builder: (context) {
                      final hasLoc = state.hasValidLocation;
                      final availCount = state.getServiceAvailabilityCount(service.id);
                      final hasExperts = hasLoc && availCount > 0;
                      final availText = state.getServiceAvailabilityText(service.id);
                      return Container(
                        margin: const EdgeInsets.only(top: 8),
                        padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 4),
                        decoration: BoxDecoration(
                          color: hasExperts ? const Color(0xFFECFDF5) : const Color(0xFFF8FAFC),
                          borderRadius: BorderRadius.circular(6),
                          border: Border.all(
                            color: hasExperts ? const Color(0xFFA7F3D0) : const Color(0xFFE2E8F0),
                            width: 1,
                          ),
                        ),
                        child: Row(
                          ...
                          children: [
                            Container(
                              ...
                              height: 7,
                              decoration: BoxDecoration(
                                color: hasExperts ? const Color(0xFF10B981) : const Color(0xFF94A3B8),
                                shape: BoxShape.circle,
                              ),
                            ),
                            const SizedBox(width: 6),
                            Flexible(
                              child: Text(
                                hasExperts ? '🟢 $availText' : availText,
                                style: TextStyle(
                                  fontSize: 11.5,
                                  fontWeight: FontWeight.w600,
                                  color: hasExperts ? const Color(0xFF047857) : const Color(0xFF64748B),
                                ),
                                maxLines: 1,
                                overflow: TextOverflow.ellipsis,
                              ),
