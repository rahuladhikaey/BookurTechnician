...
class _BookingStatusMapScreenState extends ConsumerState<BookingStatusMapScreen> {
  ...
  // Coordinate state
  LatLng _userPos = const LatLng(23.2500, 88.5500);
  LatLng? _techPos;
  ...
  void _initBookingData() {
    ...
    final fallbackLat = appState.selectedLatitude ?? appState.profile.primaryAddress?.latitude;
    final fallbackLng = appState.selectedLongitude ?? appState.profile.primaryAddress?.longitude;
    if (fallbackLat != null && fallbackLng != null) {
      _userPos = LatLng(fallbackLat, fallbackLng);
    }
  ...
  void _syncCoordinates() {
    ...
    final double? uLat = (_booking['latitude'] ??
            _booking['customerLatitude'] ??
            _booking['userLat'] ??
            _booking['lat'] ??
            _booking['customer']?['latitude'] ??
            _booking['addressObj']?['latitude']) != null
        ? ((_booking['latitude'] ??
                _booking['customerLatitude'] ??
                _booking['userLat'] ??
                _booking['lat'] ??
                _booking['customer']?['latitude'] ??
                _booking['addressObj']?['latitude']) as num)
            .toDouble()
        : fallbackLat;

    final double? uLng = (_booking['longitude'] ??
            _booking['customerLongitude'] ??
            _booking['userLng'] ??
            _booking['lng'] ??
            _booking['customer']?['longitude'] ??
            _booking['addressObj']?['longitude']) != null
        ? ((_booking['longitude'] ??
                _booking['customerLongitude'] ??
                _booking['userLng'] ??
                _booking['lng'] ??
                _booking['customer']?['longitude'] ??
                _booking['addressObj']?['longitude']) as num)
            .toDouble()
        : fallbackLng;

    final double? tLat = (_booking['technicianLatitude'] ??
            _booking['techLat'] ??
            _booking['technician']?['latitude']) != null
        ? ((_booking['technicianLatitude'] ??
                _booking['techLat'] ??
                _booking['technician']?['latitude']) as num)
            .toDouble()
        : null;

    final double? tLng = (_booking['technicianLongitude'] ??
            _booking['techLng'] ??
            _booking['technician']?['longitude']) != null
        ? ((_booking['technicianLongitude'] ??
                _booking['techLng'] ??
                _booking['technician']?['longitude']) as num)
            .toDouble()
        : null;
