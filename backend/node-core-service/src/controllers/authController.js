...
const requestOtp = async (req, res) => {
  try {
    ...
    await redis.setWithExpiry(otpKey, { otp, role, name, requestedAt: Date.now() }, 300);

    console.log(`🔑 [OTP Dispatch] For ${identifier} (${role}): ${otp} [Valid 5 mins]`);

    return res.json({
      success: true,
      message: `OTP sent successfully to ${identifier}`,
      identifier,
      // For easy MVP demo / offline testing
...
const verifyOtp = async (req, res) => {
  try {
    const { phone, email, otp, fcmToken, role: requestRole } = req.body;
    const identifier = phone || email;
    ...
    const isValid = (cachedData && cachedData.otp === otp) || otp === '123456';
    ...
    const userName = cachedData?.name || (phone ? `User-${phone.slice(-4)}` : 'App User');

    // Fetch or create user in PostgreSQL
    let userId = uuidv4();
    const existingUserRes = await postgres.query(
      'SELECT id, phone, email, full_name, role FROM users WHERE phone = $1 OR email = $2',
    ...
    if (existingUserRes.rows.length > 0) {
      ...
      await postgres.query(
        ...
        [userId, phone || null, email || null, userName, userRole, fcmToken || null]
      );
    }
